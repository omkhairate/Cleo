...
final class ScreenContextCapture {
    ...
    func captureSelectionContext(from selectedText: String?) -> OverlayVisualContext? {
    ...
    func captureSelectionContext() -> OverlayVisualContext? {
        captureSelectionContext(from: currentSelectedTextSnapshot())
    }

    func capturePointerContext(at point: NSPoint) -> OverlayVisualContext? {
        ...
        let regionDescription = regionDescription(
        ...
        return OverlayVisualContext(
            source: "pointer-context",
            summary: summary,
            selected_text: nil,
            ocr_text: combinedOCRText(focusOCR: cleanedFocusOCR),
    ...
    func captureAroundPointer(at point: NSPoint) async -> OverlayVisualContext? {
        if let selectionContext = captureSelectionContext() {
            return selectionContext
        }
        return capturePointerContext(at: point)
    }
    ...
    private func combinedOCRText(focusOCR: String?) -> String? {
        var sections: [String] = []
        if let focusOCR, !focusOCR.isEmpty {
            sections.append("Center focus OCR:\n\(focusOCR)")
        }
    ...
    private func regionDescription(
        captureFrame: CGRect,
        pointer: NSPoint,
        appName: String?,
        scope: String
